...
/// Pantalla que muestra la agenda del usuario al que
/// estamos supervisando. En el drawer, solo muestra la
/// opcion del perfil para el caso de que queramos
/// modificar algo
class PantallaAgendaSupervisor extends StatefulWidget {
  const PantallaAgendaSupervisor({super.key});

  @override
  State<PantallaAgendaSupervisor> createState() => _PantallaAgendaSupervisorState();
}

class _PantallaAgendaSupervisorState extends State<PantallaAgendaSupervisor> {
  
  // INSTANCIA BASE DE DATOS LOCAL
  BDHelper bdHelper = BDHelper();
  
  // INSTANCIA A LOS MODELOS MEDICAMENTO Y VISITA MEDICA PARA USAR SUS METODOS
  ...
  Widget build(BuildContext context) {
    ...
    Future<List<VisitaMedica>> recuperarVisitasMedicas() async{
      // MODO REMOTO
      if(modoTrabajo.modoLocal){
        return v.getVisitasMedicasUsuario(usuarioIniciado.id);
      }
      // MODO LOCAL
      else{
        return await bdHelper.getVisitasMedicasUsuario(usuarioIniciado.id);
      }
    }
          ...
          child: FutureBuilder(
            ...
            builder: (context, AsyncSnapshot<List<VisitaMedica>> snapshot){
              if(snapshot.hasData){
                ...
                // TEXTO NO HAY VISITAS MEDICAS Y AÑADIR UNA
                return const Center(
                  child: Padding(
                    padding: EdgeInsets.all(8.0),
                    child: Text(
